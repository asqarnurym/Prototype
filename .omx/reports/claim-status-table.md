...
| Midpoint word grouping reduces boundary-drop cases | PARTIALLY VERIFIED | `tests/unit/test_word_grouper.py`, `pipeline/word_grouper.py` | kept in softened form |
...
| Human accessibility benefits are proven by this prototype | NOT VERIFIED | no user study / no human ratings | explicitly not claimed |
| Qualitative slide/screencast examples prove user outcomes or semantic correctness | NOT VERIFIED | spot checks only | weakened to illustrative observations |
| Prototype is deployable in general production settings | NOT VERIFIED at general level | only local workstation/corpus evidence | weakened to “prototype-stage feasibility on the tested hardware” |
