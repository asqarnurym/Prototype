...
## 13. Manuscript Quality Gate

For conference submission readiness, the project now includes a lightweight
pre-submission toolchain for `academic_paper/`:

1. `scripts/paper_sanity_check.py`
2. `scripts/build_paper.ps1`
3. `scripts/pre_submission_check.ps1`

What this gate enforces:

- no obvious manuscript leakage of local/internal runtime markers
  (paths, localhost URLs, run IDs, draft placeholders);
- reproducible PDF build (`pdflatex -> bibtex -> pdflatex -> pdflatex`);
- page-limit guard (default: 6 pages);
- fail-fast behavior when undefined references/citations remain.

Source-of-truth usage instructions are documented in:

- `README.md` (paper pre-submission automation section);
- `USAGE.md` (manuscript workflow);
- `TESTING.md` (non-pytest manuscript checks).

Optional paper-methodology diagnostics are generated by:

1. `scripts/analyze_scene_selection_ablation.py`
2. `scripts/extract_scene_case_notes.py`

These scripts reuse the pinned paper evaluation run and saved B1 artifacts. They
write post-hoc ablation and qualitative case notes to
`evaluation/paper_extensions/` without re-running hosted model calls. The main
The current manuscript entrypoint is `academic_paper/main_full.tex`; the earlier
short IEEE variant was removed after consolidation into the full version.
